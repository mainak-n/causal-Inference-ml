...
st.markdown("""
    ...
    .stTabs [data-baseweb="tab"] {
        flex-grow: 1; /* This forces equal width */
...
def generate_pdf(ate, lower, upper, p_val, r2, treat, out):
    ...
    # Handle NaN p-values gracefully in report
    if np.isnan(p_val):
        sig_txt = "Could not calculate (Collinear Data)"
    else:
...
def run_analysis_logic(df, treatment, outcome, controls, time_col=None, date_val=None):
    ...
    # 1. Construct DataFrame for OLS
    try:
        X_df = pd.DataFrame(X, index=df.index)
        # Ensure column names match features list length
        ...
        # Fallback if OLS fails (e.g., Singular Matrix)
        ols = None
    ...
    if st.session_state['results']:
        ...
        with t2:
            if stats:
                ...
                st.warning("Statistical model could not run (likely due to perfect collinearity).")
